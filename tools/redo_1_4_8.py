...
COMMON_PARAMS = {
    ...
    "width": 1920,
    "height": 1088,
    "negative_prompt": "blurry, 3d render, ugly, distorted, noisy, watermark, bad anatomy, crowded, photo, realistic photograph",
    "override_settings": {
...
REDO_MOCKUPS = [
    ...
    },
    {
        ...
        "title": "Maquette 04 — Zoom Exécution & Clash de Lames (Parade)",
        "seed": 918273645,
        "prompt": "Cinematic close-up 2D video game screenshot, dynamic parry strike, two ink warrior silhouettes clashing glowing luminous white swords in profile, intense radiant starburst light bloom at impact point (#FFF6E2), scattering warm floating embers (#FF9A4D), wet black ink splash and brush strokes, deep indigo dusk sky background, dramatic cinematic lighting"
    },
    {
        "filename": "maquette_08_misty_meadow_fight.png",
        "title": "Maquette 08 — Duel en Brume Matinale (Atmosphère Family C)",
        "seed": 102938475,
        "prompt": "2D side-view video game screenshot, tactical sword duel in a pale misty meadow at dusk, soft pink and coral horizon sky, heavy rolling fog occluding ground level, bruised blue ink garments dissolving into wet wash, floating glowing cyan and magenta bokeh light spheres, subtle red telegraph threat zone on ground tiles, minimalist dreamlike game UI"
    }
]

def main():
    print("Redoing mockups 01, 04, and 08...")
    for item in REDO_MOCKUPS:
        ...
        try:
            ...
            with open(out_path, 'wb') as f:
                f.write(img_data)
            print(f"    Successfully regenerated {out_path} ({len(img_data)} bytes)")
        except Exception as e:
            print(f"    ERROR regenerating {filename}: {e}")

    # Re-build README.md
    ALL_MOCKUPS = [
        ("maquette_01_combat_screen.png", "Maquette 01 — Écran de Combat Tactique (Vue Lane 15 cases)", REDO_MOCKUPS[0]["prompt"]),
        ...
        ("maquette_04_execution_parry.png", "Maquette 04 — Zoom Exécution & Clash de Lames (Parade)", REDO_MOCKUPS[1]["prompt"]),
        ...
        ("maquette_08_misty_meadow_fight.png", "Maquette 08 — Duel en Brume Matinale (Atmosphère Family C)", REDO_MOCKUPS[2]["prompt"]),
    ...
    with open(readme_path, "w", encoding="utf-8") as f:
        ...
        f.write("- **Steps** : `8` | **Sampler** : `Euler` | **Schedule type** : `Simple` | **CFG Scale** : `1` | **Size** : `1920x1088`\n\n")
        f.write("---\n\n")
