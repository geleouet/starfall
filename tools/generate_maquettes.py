...
MOCKUPS = [
    ...
    {
        "filename": "maquette_03_atlas_world_map.png",
    ...
    },
    {
        ...
        "prompt": "Game UI mockup, cinematic close-up execution camera view, two duellists clashing glowing swords in profile, explosive star-shaped light bloom at contact point (#FFF6E2), scattering warm embers (#FF9A4D), ink flecks shedding from dark cloth, soft blurred ink wash background, epic dramatic duel moment, video game screenshot"
    },
    {
        "filename": "maquette_05_character_inspection.png",
    ...
    {
        "filename": "maquette_06_victory_screen.png",
    ...
    {
        "filename": "maquette_07_deck_grimoire.png",
    ...
    },
    {
        ...
        "prompt": "Game UI mockup, tactical 2D turn-based duel in a pale fog-filled meadow at dusk, pink and salmon horizon sky, bruised blue ink garments dissolving into mist, cyan and magenta floating bokeh motes, subtle red telegraph threat zone on ground tiles, minimalist game UI"
    },
    {
        "filename": "maquette_09_title_screen.png",
    ...
    {
        "filename": "maquette_10_defeat_screen.png",
...
def generate_all():
    summary_data = []
    print(f"Starting generation of {len(MOCKUPS)} mockups to {OUTPUT_DIR}...")
    for idx, item in enumerate(MOCKUPS, 1):
        ...
        print(f"[{idx}/{len(MOCKUPS)}] Generating '{title}' -> {filename}...")
        try:
    ...
    # Generate README.md in tmp/maquettes
    ...
    with open(readme_path, "w", encoding="utf-8") as f:
        ...
        f.write("---\n\n")
        f.write("## Galerie des Maquettes :\n\n")
        for item in summary_data:
            f.write(f"### {item['index']}. {item['title']}\n")
            f.write(f"- **Fichier** : [{item['filename']}](file:///{item['filepath'].replace('\\\\', '/')})\n")
            f.write(f"- **Prompt** : `{item['prompt']}`\n\n")
            f.write(f"![{item['title']}](file:///{item['filepath'].replace('\\\\', '/')})\n\n")
            f.write("---\n\n")

    print(f"Generation complete! Catalogue saved to {readme_path}")
