...
rem Ce controle vit donc a cote, comme verifyRender vit hors de Â« test Â» : il demande un ecran, et il
...
rem :expect, parce que Â« shift Â» decale aussi %0 : apres deux shift, %~dp0 vaut le repertoire courant
rem et non celui du script. Mesure : SANS-SHIFT donne le dossier du script, APRES-2-SHIFT donne
rem C:\Windows\. Ca marchait par accident, grace au Â« cd /d Â» ci-dessus -- l'ecriture disait une
rem chose et faisait l'autre.
set "HERE=%~dp0"

rem Sans cela, un echec attendu suspendrait ce script sur Â« Appuyez sur une touche Â».
...
rem deux operandes purs suffit. Et la structure Â« si le statique echoue, on saute le dynamique Â»
...
rem passent tous, si bien que la branche Â« if defined GAMEARGS goto :with_arguments Â» n'etait jamais
...
rem Le Â« pause Â» de fin ne doit s'executer QUE sans argument : lance par un script, il suspendait
...
if errorlevel 1 (
  echo   ECHEC la garde du Â« pause Â» a disparu de run.bat : un script pourrait s'y suspendre
  set /a FAILURES+=1
) else (
  echo   OK   la garde du Â« pause Â» est en place ^(controle statique^)
)
