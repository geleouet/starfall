...
public class DuelScene implements Scene, SceneProbe {
     ...
     * {@code headPx} pixels: 0 at or below 26 px (a 2 px eye there is shimmer, and
     * 4b.0 wants a wide figure to have the SUGGESTION of a face, not a face), 1 at
     * or above 44 px, smoothstep between. Static and pure so the LOD claim is
     * testable without a GL context.
     */
    static float detailFade(float headPx) {
        float u = Math.max(0f, Math.min(1f, (headPx - 26f) / 18f));
        return u * u * (3f - 2f * u);
    }
